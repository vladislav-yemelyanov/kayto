...
mod tests {
    ...
    /// Verifies `anyOf/oneOf` are rendered as TypeScript unions.
    #[test]
    fn renders_unions_for_any_of() {
        let schema = SchemaType::AnyOf(vec![
            primitive(PrimitiveType::String),
            primitive(PrimitiveType::Integer),
        ]);
        assert_eq!(schema_to_ts(&schema), "(string) | (number)");
    }

    /// Verifies `allOf` is rendered as TypeScript intersections.
    #[test]
    fn renders_intersections_for_all_of() {
        let schema = SchemaType::AllOf(vec![
            primitive(PrimitiveType::String),
            primitive(PrimitiveType::Boolean),
        ]);
        assert_eq!(schema_to_ts(&schema), "(string) & (boolean)");
    }

    /// Verifies unknown schemas are explicitly represented as `unknown`.
    #[test]
    fn renders_unknown_for_unknown_schema_type() {
        assert_eq!(schema_to_ts(&SchemaType::Unknown), "unknown");
    }
}
